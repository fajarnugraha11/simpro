$(function(){
    ...
    // DATATABLE
    var oTable = $('.list-city table').DataTable( {
        "iDisplayLength" : 10
    ...
    // Select2 Province
    $('.select2-province-id').each(function(){
        $(this).select2({
            width:"100%"
            ,placeholder: "Choose Province.."
            ,ajax: {
                url: vars.province.ajax_province
                ,dataType: 'json'
        ...
        }).select2('data', {id:vars.province.data.id,text:vars.province.data.name});
    });

    // A CLICK
    $('.list-city tbody').on('click', 'tr td a', function(){
        ...
        if(type == 'aRemove'){
            $('#delete_modal input[name="city_id"]').val(id);
    ...
    $('#form-edit-city').submit(function(){
        if($(this).valid()) {
            var validator = $(this).validate();
            validator.resetForm();
            $('#btn-save').attr('disabled', 'disabled');
...
function ajaxEdit(id){
    ...
    $.post(url, {'id':id}, function(callback){
        if(callback.status == 1) {
            ...
            $('#edit-form input[name="province_id"]').val(callback.data.province_id);
            ...
            $('#select2 input[name="province_id"]').select2('data', {id:callback.data.province_id,text:callback.data.province_name});
        }
        else {
            console.log('failed');
        }
...
function ajaxView(id){
    ...
    $.post(url, {'id':id}, function(callback){
        if(callback.status == 1) {
            //console.log(callback.data);
            $('#view-form #province_id').html(callback.data.province_id);
            $('#view-form #city_name').html(callback.data.name);
