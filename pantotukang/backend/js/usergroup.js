$(function(){
    ...
    // DATATABLE
    var oTable = $('.list-usergroup table').DataTable( {
        "iDisplayLength" : 10
    ...
    // A CLICK
    $('.list-usergroup tbody').on('click', 'tr td a', function(){
    ...
    // FORM SUBMIT
    $('#form-add-usergroup').submit(function(){
        if($(this).valid()) {
    ...
    $('#form-edit-usergroup').submit(function(){
        if($(this).valid()) {
            // var validator = $(this).validate();
            // validator.resetForm();
            $('#btn-save').attr('disabled', 'disabled');
